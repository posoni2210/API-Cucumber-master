...
public class ViewBookingDetailsStepdefinition {
	...
	public void userShpuldGetTheResponseCode(Integer statusCode) {
		assertEquals("Response is other than expected",Long.valueOf(statusCode), Long.valueOf(context.response.getStatusCode()));
	}
	...
	public void userValidatesResponseWithJSONSchema(String schemaName) {
		context.response.then().assertThat().body(JsonSchemaValidator.matchesJsonSchema(new File("C:\\Users\\posoni\\Downloads\\API-Cucumber-master\\src\\test\\resources\\schemas\\"+schemaName)));
		LOG.info("Successfully Validated schema from ");
	}
